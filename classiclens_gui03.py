...
def open_google_search(keyword):
    webbrowser.open(f"https://www.google.com/search?q={keyword}+음악의 특징")
...
def open_image():
    ...
    info = period_info.get(pred, {"kor": pred, "desc": "", "composer": ""})

    result_label.config(
        text=f"{info['kor']}의 곡으로 ({conf + 50:.1f}% 예측되었습니다)"
    )
    desc_label.config(text=info["desc"])
    composer_label.config(text=f"대표 작곡가: {info['composer']}")

    search_btn.config(command=lambda: open_google_search(info["kor"]))
...
desc_label = tk.Label(root, text="", wraplength=480, font=("Pretendard", 11), justify="center", bg="#f8f9fa")
